        ...
        <div style={{display: 'flex'}}>  
            ...
            <div className="favorite-products">
                ...
                {selectedItems.length > 0 && (
                    ...
                    <button className="favorite-product__btn favorite-product__btn--add" onClick={handleAddToCart} > 
                        <FaCartPlus/> Thêm vào giỏ hàng</button>
                    </div>
                )}
                ...
                <table className="favorite-product__table">
                    ...
                    {productsToShow.map(product => (
                        ...
                        <td>
                            <img className="favorite-product__image" src={product.image} alt={product.name} />
                        </td>
                        <td>{product.name}</td>
                        <td className="favorite-product__price">{product.price.toLocaleString()}₫</td>
                        <td className="favorite-product__original-price">{product.originalPrice.toLocaleString()}₫</td>
                        <td style={{display: '',alignItems: 'center',justifyContent: 'space-between'}}>
                            <button className="favorite-product__btn favorite-product__btn--add" onClick={handleAddToCart} ><FaCartPlus/> </button>
                            <button className="favorite-product__btn favorite-product__btn--remove"onClick={() => handleRemoveFavorite(product.id)} ><FaRegTrashAlt /> </button>
                        </td>
                        </tr>
                    ))}
                    </tbody>
