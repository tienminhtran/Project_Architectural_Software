...
const ChatBox = ({ onClose }) => {
  ...
  const getIntent = (msg) => {
    ...
    if (lower.includes("liên hệ") || lower.includes("nhân viên"))
  ...
  const searchProductsByQuery = async (query) => {
    const res = await axios.get(
      `http://localhost:8080/api/v1/products/search/${encodeURIComponent(
        query
      )}`
    );
    return res.data?.response?.values || [];
    ...
    try {
      ...
      if (intent === "search_by_price") {
        ...
        if (values.length > 0) {
          setChat((prev) => [
            ...prev,
            {
              ...
              text: `📋 Thông số sản phẩm "${values[0].productName}": ${
                values[0].specs || "Hiện chưa có thông tin chi tiết."
              }`,
            },
          ]);
        } else {
          ...
          setChat((prev) => [
            ...prev,
            { sender: "bot", text: `🤖 Trợ lý AI: ${geminiText}` },
          ]);
        }
        ...
        setChat((prev) => [
          ...
          {
            sender: "bot",
            text: "🌟 Đây là các sản phẩm được đánh giá cao nhất:",
          },
        ]);
        setProducts(values);
        ...
        setChat((prev) => [
          ...prev,
          { sender: "bot", text: "🔥 Đây là các sản phẩm bán chạy nhất:" },
        ]);
        setProducts(values);
      }
      ...
      setChat((prev) => [
        ...prev,
        { sender: "bot", text: "❗ Lỗi hệ thống. Vui lòng thử lại sau." },
      ]);
    }
    ...
    <div className="chat-box">
      ...
      {!expanded ? (
        <div className="chat-body">
          ...
          <button className="chat-button" onClick={() => setExpanded(true)}>
            Hỏi về sản phẩm
          </button>
          <button
            ...
            onClick={() => {
              setChat((prev) => [
                ...prev,
                { sender: "user", text: "Tôi muốn kiểm tra đơn hàng" },
              ]);
              setExpanded(true);
          ...
          <button
            ...
            onClick={() => {
              setChat((prev) => [
                ...prev,
                { sender: "user", text: "Tôi muốn liên hệ nhân viên" },
              ]);
              setExpanded(true);
