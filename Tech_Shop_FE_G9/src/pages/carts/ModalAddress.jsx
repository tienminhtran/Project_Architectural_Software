...
const ModalAddress = ({ isOpen, onClose, onSelect, userId }) => {
    ...
    const handleConfirm = () => {
        ...
        if (selected) {
            onSelect(formatAddress(selected));
            onClose();
        }
    };
        ...
        <div className="modal-backdrop">
            <div className="modal-container">
                <div className="modal-header-modern">
                    <h2 className="modal-title"><FaMapMarkerAlt /> Chọn địa chỉ giao hàng</h2>
                    <h6> ID: {userId}</h6>
                    <button
                ...
                <div className="address-list-modern">
                    {addressData.length > 0 ? (
                        addressData.map((addr) => (
                            <div key={addr.id} className={`address-card ${selectedId === addr.id ? 'selected' : ''}`}>
                                ...
                                <button className="edit-btn" onClick={() => handleEdit(addr)}>Sửa</button>
                            </div>
                        ))
                    ) : (
