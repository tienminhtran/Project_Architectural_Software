...
const FormProduct = () => {
    ...
    const [product, setProduct] = useState(
        normalizeInitialState(productFromLocation)
    ); // Khởi tạo giá trị cho form data từ productFromLocation
    ...
    React.useEffect(() => {
        if (productFromLocation) {
            let imagePath;
            if (
                productFromLocation.images &&
                productFromLocation.images.length > 0
            ) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (product.id) {
                await updateProduct({ formData: formData, id: product.id });
                navigate("/common/products"); // Chuyển hướng về trang danh sách sản phẩm
            } else {
                await createProduct({ formData: formData });
            }
        } catch (error) {
                                                ...
                                                src={
                                                    typeof file === "string"
                                                        ? file.startsWith("http")
                                                            ? file
                                                            : `/images/product/${handleSlitFileName(
                                                                file
                                                            )}`
