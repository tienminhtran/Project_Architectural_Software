...
export default function UserAccess() {
  ...
  const handleCancelAccounts = async () => {
    ...
    if (result.isConfirmed) {
      ...
      if (errors.length > 0) {
        Swal.fire(
          "Lỗi",
          `Hủy tài khoản thất bại với ${errors.length} user. Vui lòng thử lại!`,
          "error"
        );
  ...
  const handleSendBatchEmail = async () => {
    ...
    if (errors.length > 0) {
      Swal.fire(
        "Lỗi",
        `Có ${errors.length} email gửi không thành công. Vui lòng thử lại!`,
        "error"
      );
      console.error(errors);
    } else {
      Swal.fire(
        "Thành công",
        "Đã gửi email thông báo đến tất cả người dùng!",
        "success"
      );
    }
  };
              ...
              {data.length > 0 ? (
                data.map((user) => (
                  <tr key={user.id}>
                    ...
                    {includeOrder && (
                      <td style={styles.td}>{user.orderCount ?? 0}</td>
                    )}
                    <td style={styles.td}>
                      {user.emailNotificationDate
                        ? new Date(user.emailNotificationDate).toLocaleString()
                        : "Chưa gửi mail"}
                    </td>
                  </tr>
                ))
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {renderTable(
        filteredUsers.length > 0 ? filteredUsers : userNoOrders,
