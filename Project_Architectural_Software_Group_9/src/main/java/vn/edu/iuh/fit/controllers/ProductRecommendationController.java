 ...
 */
@RestController
@RequestMapping("/api/v1/products-recommendation")
public class ProductRecommendationController {
    ...
    @GetMapping("/all")
    public ResponseEntity<BaseResponse<?>> getAllProducts() {
        List<ProductRecommendationResponse> productRecommendationResponses= productRecommendationService.getAllProducts();
        if (productRecommendationResponses.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(BaseResponse.builder().status("SUCCESS").message("Get all products").response(productRecommendationResponses).build());
    }
}
